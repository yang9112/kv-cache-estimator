...
- **Model presets grouped by family** — DeepSeek, GLM, Qwen, LLaMA, Kimi, MiniMax, Mixtral, and more
...
Additionally, the vLLM budget calculator estimates how many tokens can be served concurrently:

```
Usable VRAM = GPU Memory x Utilization
KV Budget   = Usable VRAM - Model Weights - Overhead
...
- **模型预设按系列分组** — DeepSeek、GLM、Qwen、LLaMA、Kimi、MiniMax、Mixtral 等
- **vLLM KV 预算估算** — 根据单卡显存容量和占用率，计算可用 KV Cache 上限和最大并发 Token 数
- **导入 config.json** — 从 HuggingFace 拖入模型配置文件，自动填充架构参数
- **并行策略** — 张量并行 (TP) 和流水线并行 (PP) 的多卡显存估算
